...
form.addEventListener('submit', function (event) {
  ...
  document
    ...
    .addEventListener('click', async function () {
      ...
      // Fix date format: append full timestamp
      const dateInput = document.getElementById('date').value
      const fullDate = `${dateInput} 00:00:00` // or use current time
      formData.set('date', fullDate)

      try {
        const response = await fetch('/pim-system/product/add', {
          method: 'POST',
